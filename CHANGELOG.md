...
- `PrepareItemEnchantEvent` for an enchanting table preparing offers for an item, reporting the enchanting `inventory`, the `enchanter`, the `enchant_block`, the `item`, the three `offers` and the bookshelf `enchantment_bonus`, and carrying the new `EnchantmentOffer` type (`enchants`, `cost`). The offers are writable: replace one to change what its button applies and costs. `offers` hands back a copy, so assign the whole list back rather than editing what you read from it. It can fire more than once while the enchanting table is open. Bedrock publishes all three offers or none, so setting any offer to `None` removes all of them, as does cancelling.
